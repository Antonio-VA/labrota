...
import { SendHorizonal, CheckCircle2, XCircle, X, Sparkles } from "lucide-react"
import { useRef, useEffect, useState, useTransition } from "react"
import ReactMarkdown from "react-markdown"
import { upsertAssignment, regenerateDay, publishRota, unlockRota, copyPreviousWeek } from "@/app/(clinic)/rota/actions"
...
function ProposalCard({ proposal }: { proposal: Proposal }) {
  ...
  function handleApply() {
    startTransition(async () => {
      ...
      try {
        ...
        if (ok) {
          setStatus("done")
          window.dispatchEvent(new Event("labrota:refresh"))
          router.refresh()
        } else if (!error) {
          setStatus("pending")
        }
  ...
  return (
    <div className="rounded-lg border border-primary/20 bg-primary/5 px-3 py-2.5 text-[13px]">
      <p className="font-medium mb-1.5">{proposal.description}</p>

      {error && (
        <div className="flex items-start gap-1.5 text-destructive text-[12px] mb-1.5">
  ...
  return (
    <>
      ...
      <button
        onClick={onToggle}
        className={cn(
          "fixed bottom-6 right-6 z-30 size-14 rounded-full shadow-lg flex items-center justify-center transition-all",
          "hidden lg:flex",
