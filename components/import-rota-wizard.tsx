...
import { Upload, X, FileText, FileSpreadsheet, Image, Loader2, CheckCircle2, AlertTriangle, ChevronRight, ChevronLeft, Check, ArrowRight } from "lucide-react"
...
interface ExtractedDayOff {
  staff_name: string
  date: string
}

interface ExtractedRota {
...
// ── File processing (shared with import-wizard) ─────────────────────────────
...
function matchStaff(fileName: string, staffList: DbStaff[]): StaffMatch {
...
function matchShift(fileCode: string, shiftList: DbShift[]): ShiftMatch {
...
export function ImportRotaWizard() {
  ...
  async function handleExtract() {
    ...
    try {
      ...
      // Fallback: use inline fetch via server component pattern
      // For now we fetch via a simpler approach
