...
export function SettingsNotifications({
  ...
  function handleBulkToggle(group: "internal" | "external", enable: boolean) {
    ...
    for (const r of targets.filter((r) => r.enabled !== enable)) handleToggle(r)
  }
  ...
  function handleAdd() {
    ...
    addExternalRecipient(email, newName.trim()).then((result) => {
      ...
      setRecipients((prev) => [...prev, {
        id: crypto.randomUUID(), userId: null, email,
        name: newName.trim() || email, enabled: true, isExternal: true,
      }])
  ...
  function handleRemove(r: RecipientRow) {
    ...
    removeExternalRecipient(r.id).then((result) => {
      ...
      setRemovingIds((prev) => { const next = new Set(prev); next.delete(r.id!); return next })
    })
  }
  ...
  return (
    <div className="rounded-lg border border-border bg-background overflow-hidden">
      ...
      <div className="px-5 py-4 border-b border-border">
        <p className="text-[14px] font-medium mb-0.5">{t("title")}</p>
        <p className="text-[12px] text-muted-foreground">{t("description")}</p>
      </div>

      {/* Email format — inline row */}
      <div className="px-5 py-3 border-b border-border flex items-center justify-between">
        <div>
          <p className="text-[13px] font-medium">{t("emailFormat")}</p>
          <p className="text-[11px] text-muted-foreground">{t("emailFormatDesc")}</p>
        </div>
        <div className="flex rounded-md border border-border overflow-hidden shrink-0 ml-4">
      ...
      <div className="px-5 py-2.5 border-b border-border bg-muted/30 flex items-center justify-between">
        <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wide">{t("orgUsers")}</p>
      ...
      {internalRecipients.length === 0 ? (
        <div className="px-5 py-5 text-center text-[13px] text-muted-foreground border-b border-border">{t("noUsers")}</div>
      ) : (
          ...
          return (
            <div
              ...
              className={cn(
                ...
                i < internalRecipients.length - 1 ? "border-b border-border/40" : "border-b border-border"
              )}
            >
      ...
      {/* External emails */}
      <div className="px-5 py-2.5 border-b border-border bg-muted/30 flex items-center justify-between">
        <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wide">{t("externalEmails")}</p>
      ...
      {externalRecipients.map((r, i) => {
        ...
        return (
          <div
            ...
            className={cn(
              "px-5 py-2.5 flex items-center gap-3",
              i < externalRecipients.length - 1 ? "border-b border-border/40" : "border-b border-border"
            )}
          >
            <div className="flex-1 min-w-0">
              {hasName ? (
      ...
      <div className="px-5 py-3 flex items-end gap-2 flex-wrap">
        <div className="flex flex-col gap-1">
          <label className="text-[11px] text-muted-foreground font-medium">{t("emailLabel")}</label>
          <Input
            type="email"
            value={newEmail}
        ...
        <div className="flex flex-col gap-1">
          <label className="text-[11px] text-muted-foreground font-medium">{t("fullNameLabel")}</label>
          <Input
            type="text"
            value={newName}
