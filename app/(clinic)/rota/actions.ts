...
export async function getRotaWeek(weekStart: string): Promise<RotaWeekData> {
  ...
  const [rotaResultFull, labConfigResult, leavesResult, shiftTypesRes, tecnicasRes, departmentsRes, rulesRes] = await Promise.all([
    ...
    supabase.from("rota_rules").select("type, enabled, staff_ids, params").eq("enabled", true).in("type", ["restriccion_dia_tecnica", "supervisor_requerido"]) as unknown as Promise<{ data: { type: string; enabled: boolean; staff_ids: string[]; params: Record<string, unknown>; expires_at: string | null }[] | null }>,
  ])
